...
def process_frame(frame):
    ...
    for box in results.boxes:
        ...
        color = (0,255,0) if label.lower()=="person" else (0,0,255)
...
def define_safe_zone():
    ...
    cv2.namedWindow("Draw Safe Zone - Click points, press Enter when done")
    cv2.setMouseCallback("Draw Safe Zone - Click points, press Enter when done", set_safe_zone)
    while True:
        ret, frame = cap.read()
        if not ret:
            break
        if safe_zone:
            cv2.polylines(frame, [np.array(safe_zone, np.int32)], isClosed=False, color=(0, 255, 255), thickness=2)
            for point in safe_zone:
                cv2.circle(frame, point, 5, (0,255,255), -1)
        cv2.imshow("Draw Safe Zone - Click points, press Enter when done", frame)
        key = cv2.waitKey(1)
        if key == 13:  # Enter key
            break
    cap.release()
...
def start_webcam():
    define_safe_zone()  # let user draw safe zone
